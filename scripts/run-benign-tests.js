...
function applyTestExclusions(excludedTests, rootDir) {
    ...
    for (let [relPath, descriptions] of Object.entries(fileMap)) {
        // 修正逻辑：如果路径没带 frontend/ 但实际上在 frontend 下，自动补齐
        ...
        try {
            ...
            descriptions.forEach(fullDesc => {
                // 算法：取全名匹配 + 取末尾 2 个单词匹配
                const words = fullDesc.split(' ').filter(w => w.length > 2);
                const lastTwo = words.slice(-2).join('.*?'); // 比如 "status.*?properly"

                // 更激进的正则：
                // 1. 考虑单引号、双引号、反引号
                // 2. 考虑 it, fit, test
                // 3. 考虑跨行
                const regex = new RegExp(`\\b(it|fit|test)\\s*\\(\\s*['"\`][^]*?(${lastTwo})[^]*?['"\`]`, 'gi');

                if (regex.test(content)) {
                    content = content.replace(regex, (match) => {
                        if (/^x/i.test(match)) return match;
                        return 'x' + match;
                    });
                    isModified = true;
                }
...
async function prepareChatBotEnvironment(projectRoot) {
    ...
    if (fs.existsSync(staticPath)) {
        ...
        if (stats.size === 0) {
            throw new Error("ChatBot data file is empty after copy!");
        }
...
async function runFrontendTestsWithPatch(excludedTests, projectRoot) {
    let modifiedFiles = new Set();
    try {
        modifiedFiles = applyTestExclusions(excludedTests, projectRoot);
        return await runFrontendTests();
    } finally {
    ...
    return new Promise((resolve) => {
        ...
        child.stdout.on('data', (data) => {
            data.toString().split('\n').forEach(line => {
                ...
                if (!isNoise || line.includes('FAILED')) {
...
async function runStandardTests() {
    ...
    for (let i = 0; i < jestFiles.length; i += BATCH_SIZE) {
        ...
        console.log(`[Jest] Batch ${Math.floor(i / BATCH_SIZE) + 1}...`);
...
async function runCypressWithServer(cypressFiles) {
    ...
    console.log(`\n[Cypress] Starting server and E2E...`);
...
async function main() {
    ...
    try {
        prepareChatBotEnvironment(projectRoot);

        // 1. Frontend
        const frontendPassed = await runFrontendTestsWithPatch(manifest.excluded || [], projectRoot);
        allPassed = frontendPassed && allPassed;

        // 2. API
        const apiPassed = await runStandardTests();
        allPassed = apiPassed && allPassed;
        ...
        if (allPassed && cypressFiles.length > 0) {
            ...
            allPassed = cypressPassed && allPassed;
        }
